<template>
    <div class="mitre-mobile-page">
        <h1 class="page-title">Mitre Mobile</h1>

        <v-client-table :columns="columns" v-model="mitreMobile" :options="options">
            <div slot="child_row" slot-scope="props">
                <div style="width: 100%;box-shadow:inset 0 0 4px rgba(0 0 0 / 13%);padding: 10px;">
                    <h6>External References</h6>
                    <table class="table-bordered" v-if="props.row.external_references && props.row.external_references.length > 0">
                        <thead>
                        <tr>
                            <th>External ID</th>
                            <th>Source Name</th>
                            <th>External URL</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="reference in props.row.external_references">
                            <td>{{ reference.external_id || "Unset"}}</td>
                            <td>{{ reference.source_name || "Unset"}}</td>
                            <td>{{ reference.url || "Unset" }}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </v-client-table>
    </div>
</template>

<script>
    import {mapState} from "vuex";

    export default {
        name: 'MitreMobile',
        components: {
        },
        data() {
            return {
                columns: ['id', 'type', 'description', 'external_references'],
                options: {
                    headings: {
                        id: 'ID',
                        type: 'Type',
                        description: 'Description',
                        external_references: 'External References',
                    },
                    editableColumns: [],
                    sortable: ['name', 'type'],
                    filterable: ['name', 'pattern', 'type']
                }
            };
        },
        methods: {
        },
        computed: {
            ...mapState('data', ['mitreMobile'])
        }
    };
</script>

<style src="./MitreMobile.scss" lang="scss" />
